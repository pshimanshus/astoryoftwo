...
def fit_on_canvas(image: np.ndarray, size: tuple[int, int]) -> np.ndarray:
    target_w, target_h = size
    h, w = image.shape[:2]
    scale = min(target_w / w, target_h / h)
    resized = cv2.resize(image, (round(w * scale), round(h * scale)), interpolation=cv2.INTER_LANCZOS4)

    # The generated slides use a warm cream paper. Sampling the corner preserves the same paper tone.
    corners = np.array(
        [
            image[0:30, 0:30],
            image[0:30, -30:],
    ...
    paper = np.median(corners, axis=0).astype(np.uint8)
    canvas = np.full((target_h, target_w, 3), paper.tolist(), dtype=np.uint8)
...
def main() -> None:
    ...
    hd_out = OUT_DIR / "hd-2160x2700"
    for folder in [source_out, ig_out, hd_out]:
    ...
    for index, filename in enumerate(SLIDES, start=1):
        ...
        ig = fit_on_canvas(image, (1080, 1350))
        hd = fit_on_canvas(image, (2160, 2700))
        ig_path = ig_out / slide_name
        hd_path = hd_out / slide_name
        write_png(ig_path, ig)
        write_png(hd_path, hd)
