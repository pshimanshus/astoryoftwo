...
Examples:
  ...
  venv/bin/python scripts/create_illustration_carousel.py --mode anthropic --story "..." --image photo.jpg
    ...
    parser.add_argument(
        ...
        help="Runtime to use. Default is codex-native and requires no external credentials.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Deprecated alias for --prepare-image-handoff. Prepares Codex image tool "
            "handoff files."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help="Prepare Codex image tool handoff files for in-session generation.",
    )
    parser.add_argument(
        "--proof-slide",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            "Image generation backend. codex-built-in prepares Codex image tool jobs; "
    ...
    if args.mode == "anthropic":
        ...
        if args.generate_images:
            print(
                ...
                "to prepare Codex image tool jobs."
            )
        ...
        if handoff_requested and args.image_backend == "codex-built-in":
            ...
            print(f"Codex image tool handoff -> {result['status']}")
            print("Use Codex image generation in-session with the written prompt files, then package outputs.")
