...
class IllustrationCarouselTests(unittest.TestCase):
    ...
    def test_codex_builtin_handoff_writes_identity_reference_prompt_files(self):
        ...
        self.assertIn("Prompt Source", instagram_prompt_text)
        self.assertIn("Paste the full prompt from `slide-01.prompt.txt`", instagram_prompt_text)
        self.assertNotIn("\n## Prompt\n", instagram_prompt_text)
        self.assertNotIn("Face identity contract", instagram_prompt_text)
        self.assertNotIn("Render this exact handwritten-style text inside the artwork", instagram_prompt_text)
